...
def download_blob_container_dir(
    ...
    if keep_structure:
        local_output_dir = (
            os.path.join(destination_dir, azure_source_dir)
            .replace("\\", os.sep)
            .replace("/", os.sep)
        )
    ...
    _LOG.info(
        f"downloading `{azure_source_dir}` from container `{container_name}` to `{local_output_dir}`"
    )
    with tempfile.TemporaryDirectory() as temp_dir:
        _LOG.info(f"downloading to temp dir first: {temp_dir}")
        os.makedirs(temp_dir, exist_ok=True)
        command = (
            f"az storage copy --recursive --only-show-errors "
        ...
        try:
            ...
            _LOG.info(
                f"download completed successfully, time: {time.time() - start_time:.0f} seconds"
            )
        except subprocess.CalledProcessError as e:
            _LOG.error(
                f"failed to download `{azure_source_dir}` from `{container_name}` to `{local_output_dir}`: {e}"
            )
            return None
        ...
        temp_output_dir = (
            os.path.join(temp_dir, container_name, azure_source_dir)
            .replace("\\", os.sep)
            .replace("/", os.sep)
        )
...
def download_meeting_subset(
    ...
    return download_blob_container_dir(
        azure_source_dir=azure_dir,
...
def download_simulated_subset(
    version: str,
    volume: Literal["200hrs", "1000hrs"],
    subset_name: Literal["train", "val"],
    destination_dir: str,
    overwrite: bool = False,
) -> Optional[str]:
    """
...
def download_models(
    destination_dir: str, pattern: Optional[str] = None, overwrite: bool = False
) -> Optional[str]:
    """
