...
def prepare_chime6(
    ...
    all_sessions = [
        Path(x).stem
        for x in glob.glob(
            os.path.join(transcriptions_dir, dset_part, "*.json")
        )
    ]
    ...
    if mic == "ihm":
        for session in all_sessions:
            audio_paths = [
                ...
                for x in glob.glob(
                    os.path.join(
                        corpus_dir, "audio", dset_part, f"{session}_P*.wav"
                    )
                )
            ]
            ...
            for idx, audio_path in enumerate(audio_paths):
                ...
                sources.append(
                    AudioSource(
                        type="file", channels=channels, source=str(audio_path)
                    )
                )
        ...
        for session in all_sessions:
            audio_paths = [
                ...
                for x in glob.glob(
                    os.path.join(
                        corpus_dir, "audio", dset_part, f"{session}_U*.wav"
                    )
                )
            ]
            ...
            for idx, audio_path in enumerate(sorted(audio_paths)):
                sources.append(
                    AudioSource(
                        type="file", channels=[idx], source=str(audio_path)
                    )
                )
    ...
    for session in all_sessions:
        with open(
            os.path.join(transcriptions_dir, dset_part, f"{session}.json")
        ) as f:
                ...
                supervisions.append(
                    SupervisionSegment(
                        ...
                        duration=add_durations(
                            end, -start, sampling_rate=CHIME_6_FS
                        ),
                        channel=channel,
                        text=segment["words"],
                        language="English",
            ...
            for idx, segment in enumerate(transcript):
                ...
                supervisions.append(
                    SupervisionSegment(
                        ...
                        start=start,
                        duration=add_durations(
                            end, -start, sampling_rate=DIPCO_FS
                        ),
                        channel=channel,
                        text=segment["words"],
                        speaker=spk_id,
...
def prepare_mixer6(
    ...
    audio_files = glob.glob(
        os.path.join(corpus_dir, "audio", dset_part, "*.flac")
    )
    assert (
        len(audio_files) > 0
    ...
    for sess in all_sessions:
        with open(
            os.path.join(transcriptions_dir, dset_part, f"{sess}.json")
        ) as f:
            transcript = json.load(f)
        if mic == "ihm":
        ...
        for idx, segment in enumerate(transcript):
            ...
            supervisions.append(
                SupervisionSegment(
                    ...
                    start=start,
                    duration=add_durations(
                        end, -start, sampling_rate=MIXER6_FS
                    ),
                    channel=channel,
                    text=segment["words"],
                    speaker=spk_id,
